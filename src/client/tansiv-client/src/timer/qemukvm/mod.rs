...
impl TimerContextInner {
    ...
    pub fn delay(&self, delay: StdDuration) {
        ...
        let tsc_freq = *self.tsc_freq.lock().unwrap();
        if let Some(target_tsc) = now_tsc.checked_add((delay_nanos as f64 * tsc_freq) as u64) {
            while unsafe { _rdtsc() } < target_tsc {
            }
        }
        else {
            debug!("delay: Overflow detected. now_tsc : {:?} delay_nanos * tsc_freq : {:?}", now_tsc, (delay_nanos as f64 * tsc_freq) as u64)
        } 
    }
}
