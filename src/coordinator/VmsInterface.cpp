...
bool sortMessages(Message *i, Message *j) { return i->sent_time < j->sent_time; }
...
double vmToSimgridTime(uint64_t seconds, uint64_t nseconds) { return seconds + nseconds * 1e-9; }

double vmToSimgridTime(vsg_time vm_time) { return vmToSimgridTime(vm_time.seconds, vm_time.nseconds); }
...
VmsInterface::~VmsInterface() { end_simulation(true, false); }
  ...
  for (auto kv : vm_sockets) {
    ...
    while (!finished) {
      if (fb_recv(vm_socket, scratch_buffer, SCRATCH_BUFFER_LEN) < 0) {
        LOG("can not receive the flags of VM " << vm_name
                                               << ". Forget about the socket that seem closed at the system level.");
