...
class VMXen(object):
    ...
    def start(self, working_dir: Path) -> Path:
        ...
        cmd = f"xl create -f domain-{self.descriptor}.cfg"
...
def fill_template(
    out: Path, template: str = "templates/deployment-xen.cfg.j2", **kwargs
):
    env = Environment(
        loader=FileSystemLoader(Path(__file__).parent / "templates"),
        autoescape=select_autoescape(),
    )
    template = env.get_template("deployment-xen.cfg.j2")
