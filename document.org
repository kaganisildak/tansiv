...
Boucle principale de SimGrid
  time = 0
  tant qu’il reste des processus actifs,
    deadline = min (time + min_latency, next_reception_time)
    si deadline == time # slot de durée nulle
      on envoie le ou les message sortant de SimGrid dans la ou les vm de destination
    sinon
      barrier(fin_slot) # les VM ont avancé jusqu'à deadline
      trier B par temps d’émission croissant
      pour chaque (m_e, t_e) dans B
        avancer SimGrid jusqu’à t_e et commencer la transmission de m_e
      B = {}
      avancer SimGrid jusqu’à deadline. Invariant: on n'a aucune réception entre time et deadline
